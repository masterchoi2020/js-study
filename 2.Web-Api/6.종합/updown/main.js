...
function makeNumberIcons() {
    ...
    return $numbers;    //numbers를 리턴해서 메인 로직함수에서 받아 준다.
}
...
function checkAnswer(eventTarget){
    ...
    makeNumberIcons();    //min, max 의 변화대로 재배치
}
...
(function(){
    ...
    $numbers.addEventListener('click', function(e){
        ...
        currentTarget: 이벤트 핸들러가 걸린 요소(div#numbers -> 부모)
        */

        // console.log('아이콘이 클릭됨!', e.target.textContent);
        
        //gameDatas에 answer프로퍼티에 사용자가 선택한 숫자를 저장
        gameDatas.answer = +e.target.textContent; //textContent 타입: String
